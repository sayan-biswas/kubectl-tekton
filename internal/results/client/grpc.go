...
const (
	GRPCContentType string = "application/grpc"
)
...
// create creates a new gRPC client.
func (client *grpcClient) create(opts *Options) (Interface, error) {
	...
	callOptions := []grpc.CallOption{
		grpc.PerRPCCredentials(&customCredentials{
			...
			ImpersonationConfig: opts.ImpersonationConfig,
		}),
	}
	...
	if u.Port() == "" {
		...
		case "http":
			u.Host = u.Host + ":80"
		default:
			return nil, errors.New("please specify port in host")
		}
	}
